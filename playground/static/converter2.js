...
(function(global, factory) {
	...
	function addAxes(axis, node, argumentInfo) {
		console.log(node.parentNode.toString(), axis, argumentInfo);
		//if ( !axis || node.axis || axis === "self::" && !argumentInfo) return;
		//if (!axis || node.axis || axis === "self::" && !argumentInfo || argumentInfo && !argumentInfo.self) return;
		if (!axis || node.axis || axis === "self::" && ( !argumentInfo || argumentInfo.ofSelector)) return;
	...
	/*function addAxes(axis, node, argumentInfo) {
		//if ( !axis || node.axis || axis === "self::" && !argumentInfo) return;
		if (!axis || node.axis || argumentInfo && argumentInfo.predicate) return;
		var text = node.parentNode.toString().trim(),
			len = text.length;
		if (len == 0 || / (?:or|\|)$/.test(text)) {
			node.axis = axis;
	...
	function processNth(name, arg, not, parNode, node) {
		...
		if (usePosition && child) {
			...
			var newNodeOwner = node.owner && node.owner !== "node()" ? node.owner : "*";
			node.owner = owner;
			node.add(str);
			//console.log(owner, str);
			...
			if (ofResult) {
				//node.owner = owner;
				//console.log(node.axis, owner, ofResult , str);
				node.add(ofResult);
				//node.add(owner);
			}
			node.add(str);
	...
	function addNthToXpath(name, arg, sibling, owner, last, usePosition) {
		...
		if (/^\d+$/.test(arg)) {
			...
			if (obj.valueA) {
				...
				if (obj.type === 'mod') str = addModulo(sibling, owner, _num, obj.valueA, 0);
	...
	function checkOfSelector(name, arg, node) {
		...
		if (rm !== null) {
			...
			//ofResult = convertArgument(nd, rm[1], 'self::', "node()", {
			//ofResult = convertArgument(nd, rm[1], '', "node()", {
				ofSelector: true,
				predicate: true,
				name: name
			});
			...
			if (nd.childNodes.length === 1) {
				...
				if (firstChild.owner === "node()") {
					...
					owner += result;
					ofResult = result;
					//console.log( getOwner(firstChild, ''), firstChild.owner);
